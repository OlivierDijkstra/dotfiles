...
chooser="--chooser-file=$out"
cwdfiles="--cwd-file=$last_selected_path_cfg"

title="Open File"

if [ "$multiple" = "1" ] && [ "$directory" = "0" ] && [ "$save" = "0" ]; then
  title="Open Files"
fi

if [ "$multiple" = "1" ] && [ "$directory" = "1" ] && [ "$save" = "0" ]; then
  title="Open Folder"
fi

if [ "$multiple" = "0" ] && [ "$directory" = "0" ] && [ "$save" = "1" ]; then
  title="Save File"
fi

if [ "$multiple" = "1" ] && [ "$directory" = "0" ] && [ "$save" = "1" ]; then
  title="Save Files"
fi

sh -c "$termcmd --title='$title' -e yazi  $chooser $cwdfiles "$path""
